...
본 약관은 `한국 디스코드봇 리스트` (이하 "회사")가 운영하는 https://koreanbots.dev 에 위치한 웹사이트(이하 "웹사이트")를 이용하거나 접속하는 모든 행위는 "서비스"에 해당되며, 이 서비스 이용에 대한 회사와 서비스 유저의 권리, 의무, 책임 등 필요한 사항을 규정함을 목적으로 합니다.
