...
서버 내에서 뮤트 처분을 받더라도 웹사이트에서는 어떠한 불이익도 받지 않습니다.  
뮤트 처분을 받은 경우 최대 1회에 한하여 문제 해결 후 재검토 요청을 하실 수 있습니다.  
- 레벨링 메시지 전송
- 접두사와 관련 없이 특정 키워드가 아닌 모든 메시지에 항상 혹은 무작위로 응답
- 다수의 메시지를 짧은 시간안에 지속적으로 전송(도배 행위)
